...
public class GenKeys {
    ...
    public GenKeys(int state, String nodeid, String configfilesLocation, String generatorsLocation,
            String publickeysLocation, String privatekeysLocation) throws InterruptedException, JsonException {
        ...
        if (state == 0) {
            ...
            try {
                Process generator = Runtime.getRuntime()
                        .exec(new String[] { "bash", "-c", "openssl genpkey -genparam -algorithm DH -out "
            ...
            try {
                Process privatekey = Runtime.getRuntime().exec(
                        new String[] { "bash", "-c", "openssl genpkey -paramfile  " + configfilesLocation.toString()
                                + "/mygenerator.pem -out " + configfilesLocation.toString() + "/myprivatekey.pem" });
                privatekey.waitFor();
            ...
            try {
                Process publickey = Runtime.getRuntime()
                        .exec(new String[] { "bash", "-c",
                                "openssl pkey -in " + configfilesLocation.toString() + "/myprivatekey.pem -pubout -out "
            ...
            try {
                ...
                Process generator = Runtime.getRuntime()
                        .exec(new String[] { "bash", "-c",
                                "echo '" + cleangen + "' > " + generatorsLocation.toString() + "/" + nodeid
                                        + "generator.pem; cat " + generatorsLocation.toString() + "/" + nodeid
                                        + "generator.pem | " + "sed -i 's/\\\\n/\\'$'\\n''/g' "
                                        + generatorsLocation.toString() + "/" + nodeid + "generator.pem" });
                generator.waitFor();
            ...
            try {
                ...
                Process pubkeymake = Runtime.getRuntime()
                        .exec(new String[] { "bash", "-c",
                                "echo '" + cleanpubkey + "' > " + publickeysLocation.toString() + "/" + nodeid
                                        + "publickey.pem; cat " + publickeysLocation.toString() + "/" + nodeid
                                        + "publickey.pem | sed -i" + " 's/\\\\n/\\'$'\\n''/g' "
                                        + publickeysLocation.toString() + "/" + nodeid + "publickey.pem" });
            ...
            // create private key from generator
            try {
                Process privatekey = Runtime.getRuntime()
                        .exec(new String[] { "bash", "-c",
                                "openssl genpkey -paramfile " + generatorsLocation.toString() + "/" + nodeid
                                        + "generator.pem -out " + privatekeysLocation.toString() + "/" + nodeid
                                        + "myprivatekey.pem" });
                privatekey.waitFor();
            ...
            try {
                Process publickey = Runtime.getRuntime()
                        .exec(new String[] { "bash", "-c",
                                "openssl pkey -in " + privatekeysLocation.toString() + "/" + nodeid
                                        + "myprivatekey.pem -pubout -out " + publickeysLocation.toString() + "/"
                                        + nodeid + "mypublickey.pem" });
                publickey.waitFor();
