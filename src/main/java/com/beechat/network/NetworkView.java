...
public class NetworkView {
    ...
    /**
     * Construct and display a new NetworkView. There should only be one NetworkView active at a time.
    ...
    public NetworkView(DigiMeshNetwork network, DigiMeshDevice device) {
        ...
        Object[][] tableData = new Object[network.getNumberOfDevices()][2]; // JTable only supports primitive arrays. There is probably a better way of doing this but... :)
        ...
        startConversationButton.addActionListener(new ActionListener() {
            /**
             * Start a new conversation with the node selected from the table.
             * TODO: Check if a conversation is already actively with the node, and if so; notify the user and do not open another ConversationForm window.
             */
            @Override
            public void actionPerformed(ActionEvent e) {
                new ConversationForm(
                        (RemoteDigiMeshDevice) network.getDevice(String.valueOf(table.getValueAt(
                                table.getSelectedRow(),0))), network, device);
            }
        });
