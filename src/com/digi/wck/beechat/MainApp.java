...
public class MainApp extends Constants{
    ...
    private static final MainApp.DataReceiveListener listener = new MainApp.DataReceiveListener();
    ...
    static TimerTask task = new TimerTask() {
        public void run() {
            if ((connected) && (secondspassed - prevmsgtimestamp > 1) && (ZIPFILESignal)) {
                if (!delayDetected) {
    ...
    public static void main(String[] args) throws Throwable {
        ...
        try {
            ...
            mkfile = Runtime.getRuntime().exec(new String[]{
                    "bash", "-c", "mkdir " + configfilesLocation + "; touch " + configfilesLocation +
                    "/fileexists; touch " + configfilesLocation + "/activeport"});
            mkfile.waitFor();
        ...
        Path[] allfolders = {generatorsLocation, privatekeysLocation, publickeysLocation, configfilesLocation,
                             downloadfilesLocation, uploadfilesLocation, deepspeechLocation, deepspeechModelsLocation};

        int i = 0;

        while (i < allfolders.length) {
            if (!Files.exists(allfolders[i])) {
                firstrun = true;
            }
            i = i + 1;
        }
                ...
                if (!(Files.isDirectory(Paths.get(deepspeechLocation + "/deepspeechenv")))) {
                    try {
                        ...
                        Process virtualenv = Runtime.getRuntime().exec(new String[]{
                                "bash", "-c", "virtualenv -p python3 " + deepspeechLocation.toString() +
                                "/deepspeechenv; source " + deepspeechLocation + "deepspeechenv/bin/activate;" +
                                " pip3 install deepspeech"});
                        virtualenv.waitFor();
                        System.out.println("DeepSpeech installed.");
                    } catch (IOException e1) {
                        System.err.println("An error has occurred while installing DeepSpeech.");
                        e1.printStackTrace();
                    }
                }
            }
                ...
                if (jarFile.isFile()) {  // Run with JAR file
                        ...
                        if (i != 0) {
                            try {
                                Process fileextractor = Runtime.getRuntime().exec(new String[]{
                                        "bash", "-c", "cd " + userDirectory + "; 7z e " + jar.getName() + " -o" +
                                        finalpath[i].toString() + " " + allresources[i]});

                                fileextractor.waitFor();
                            } catch (IOException e) {
                                System.err.println("An error has occurred while extracting " + jar.getName() + ".");
                                e.printStackTrace();
                            }
                        } else {
                            try {
                                ...
                                if (!rxtxlibcheck.exists() && !rxtxlibcheck.isDirectory()) {
                                    Process fileextractor = Runtime.getRuntime().exec(new String[]{
                                            "bash", "-c", "cd " + userDirectory + "; 7z e " + jar.getName() + " " +
                                            allresources[i].toString().substring(1, allresources[i].toString().length())
                                            + " -y; pkexec mv " + userDirectory + "/" + allresources[i].toString()
                                            .substring(allresources[i].toString().lastIndexOf("/")
                                                    + 1, allresources[i].toString().length()) + " " +
                                            finalpath[i].toString() + "/; touch " + configfilesLocation +
                                            "/libinstalled; echo ok > " + configfilesLocation + "/libinstalled;"});
                                ...
                                System.err.println("An exception has occurred."); //what is above code doing?
                                e.printStackTrace();
                            }
                        }
                    }
                    ...
                    if (url != null) {
                        try {
                                ...
                                if (!f.exists() && !f.isDirectory()) {
                                    try {
                                        Process filecopier = Runtime.getRuntime().exec(new String[]{
                                                "bash", "-c", "cd " + resource.getPath() + "; cp " +
                                                resource.getAbsolutePath() + " " + finalpath[i].toString()});
                                ...
                                if (!f.exists() && !f.isDirectory()) {
                                    try {
                                        Process sudofilecopier = Runtime.getRuntime().exec(new String[]{
                                                "bash", "-c", "cd " + resource.getPath() + "; pkexec cp " +
                                                resource.getAbsolutePath() + " " + finalpath[i].toString()});
                            ...
                            System.err.println("You should never see this. " + ex.getMessage());
                        }
                    }
                }
        ...
        try {
            Process portsetter = Runtime.getRuntime().exec(new String[]{
                    "bash", "-c", "echo \"$(ls /dev | grep \"USB\")\" > " +
                    configfilesLocation.toString() + "/activeport"});
        ...
        if (stringContainsNumber(file2string(configfilesLocation.toString() + "/activeport"))) {
        ...
        try {
            ...
            System.err.println("An exception has occured while removing lock file.\n" + err.getMessage());
            err.printStackTrace();
        }
        ...
        try {
            ...
            if (!fileexists(configfilesLocation.toString() + "/contacts.json")) {
                try {
            ...
            if ((fileexists(configfilesLocation.toString() + "/contacts.json")) && 
                    (Files.isDirectory(Paths.get(deepspeechLocation + "/deepspeechenv")))) {

                // FIXME: 5/4/20 WuK, why is this here?
                
            }

        } catch (InterruptedException e) {
            System.err.println("A process was interrupted.");
            e.printStackTrace();
        } catch (IOException e) {
            System.err.println("There was a problem creating a file or folder.");
            e.printStackTrace();
        }
        ...
        if (firstrun) {
            ...
            try {
                ...
                System.err.println("Command \"bash -c touch " + configfilesLocation);
                e.printStackTrace();
            }
        }
    ...
    static void docommands(String command, int chunklength, XBeeDevice myDevice, String uname,
                           String myPublicKey, String myGen) throws Exception, Throwable {

        //COMMANDS SECTION
        // TODO: 5/4/20 Port these to individual class files in the command directory.
        ...
        if (command.equals("chat")) {
                    ...
                    if (remote != null) {
                        if (getcontact(REMOTE_NODE_ID)[0].equals(REMOTE_NODE_ID)) {
                            ...
                            while (!didiexit) {
                                ...
                                if (message.substring(0, 4).contentEquals("exit")) {
                                    didiexit = true;
                                }
                                ...
                                Process appendtologfile = Runtime.getRuntime().exec(new String[]{
                                        "bash", "-c", "echo '" + myDevice.getNodeID() + ": " + message + "' >> " +
                                        configfilesLocation + "/" + remote.getNodeID()});
                                ...
                                Process sharedsecretfile = Runtime.getRuntime().exec(new String[]{
                                        "bash", "-c", "openssl pkeyutl -derive -inkey " + privatekeysLocation.toString()
                                        + "/" + REMOTE_NODE_ID + "myprivatekey.pem -peerkey " +
                                        publickeysLocation.toString() + "/" + REMOTE_NODE_ID +
                                        "publickey.pem | openssl sha3-256 | sed -e 's|(stdin)= ||' > "
                                        + userDirectory + "/sharedsecret"});

                                sharedsecretfile.waitFor();


                                //Create AESCrypt file
                                AESCrypt encrypter = new AESCrypt(file2string(userDirectory + "/sharedsecret"));
                                //System.out.println(file2string(userDirectory + "/sharedsecret"));

                                //Delete sharedsecret file
                                Process deletesharedsecretfile = Runtime.getRuntime().exec(new String[]{
                                        "bash", "-c", "rm -rf " + userDirectory + "/sharedsecret"});
                                deletesharedsecretfile.waitFor();

                                //Converting file to aes with shared sha3-256 secret
                                encrypter.encrypt(2, uploadfilesLocation + "/msg",
                                        uploadfilesLocation + "/msg.aes");
                                ...
                                //SENDING HEADER
                                String pubkeysend = file2string(publickeysLocation.toString() + "/" +
                                        REMOTE_NODE_ID + "mypublickey.pem");
                                ...
                                while (i < CHATHeaderbytebuffer.length) {
                                    myDevice.sendData(remote, Arrays.copyOfRange(CHATHeaderbytebuffer, i,
                                            i + chunklength));
                                ...
                                while (i < CHATFooterbytebuffer.length) {
                                    myDevice.sendData(remote, Arrays.copyOfRange(
                            ...
                            System.out.println("Contact is unknown, please have them send their " +
                                    "public and generator keys.");
                            myDevice.close();
                        }
            ...
            chunklength = 255;
            myDevice.close();
            System.out.println("\nRemember that to send files to another device, you must have their contact" +
                    " information. Restart the program and enter the 'help' command to learn how to do this.");
                    ...
                    if (remote != null) {
                        if (getcontact(REMOTE_NODE_ID)[0].equals(REMOTE_NODE_ID)) {
                            ...
                            System.out.println("Connection established with " + REMOTE_NODE_ID + ". " +
                                    "Choose your file to send, or type in 'exit' to exit:\n");
                            while (true) {
                                ...
                                int currpart = 0;
                                try {
                                    currpartString = file2string(uploadfilesLocation + "/dir-" +
                                            filetosend + "/lastpartsent");
                                ...
                                if (currpart == 1) {
                                    ...
                                    //Getting shared secret
                                    Process sharedsecretfile = Runtime.getRuntime().exec(new String[]{
                                            "bash", "-c", "openssl pkeyutl -derive -inkey " +
                                            privatekeysLocation.toString() + "/" + REMOTE_NODE_ID + "myprivatekey.pem" +
                                            " -peerkey " + publickeysLocation.toString() + "/" + REMOTE_NODE_ID +
                                            "publickey.pem | openssl sha3-256 | sed -e 's|(stdin)= ||' > " +
                                            userDirectory + "/sharedsecret"});
                                    sharedsecretfile.waitFor();
                                    //Create AESCrypt file
                                    AESCrypt encrypter = new AESCrypt(file2string
                                            (userDirectory + "/sharedsecret"));
                                    System.out.println(file2string(userDirectory + "/sharedsecret"));
                                    //Delete sharedsecret file
                                    Process deletesharedsecretfile = Runtime.getRuntime().exec(new String[]{
                                    ...
                                    //Converting file to aes with shared sha3-256 secret
                                    encrypter.encrypt(2, uploadfilesLocation + "/" + filetosend,
                                            uploadfilesLocation + "/" + filetosend + ".aes");
                                    ...
                                    try {
                                        ...
                                        Process zipfile = Runtime.getRuntime().exec(new String[]{
                                                "bash", "-c", "7z -v" + maxfilesize + "b a " + uploadfilesLocation +
                                                "/dir-" + filetosend + "/" + filetosend + ".aes.7z " +
                                                uploadfilesLocation + "/" + filetosend + ".aes"});
                                        zipfile.waitFor();

                                        Process countfiles = Runtime.getRuntime().exec(new String[]{
                                                "bash", "-c", "find " + uploadfilesLocation + "/dir-" + filetosend +
                                                "/ -type f | wc -l > " + configfilesLocation + "/filecount"});
                                        countfiles.waitFor();

                                        partstosend = Integer.parseInt(file2string(configfilesLocation +
                                                "/filecount").replace("\n", ""));
                                        ...
                                        try {
                                            ...
                                            while (i < ZIPHeaderbytebuffer.length) {
                                                myDevice.sendData(remote, Arrays.copyOfRange(ZIPHeaderbytebuffer, i,
                                                        i + chunklength));

                                                //System.out.println("Sending file info...");
                                                i = i + chunklength;
                                            }
                                            sendparts(myDevice, remote, currpart, partstosend,
                                                    filetosend, partfileLocation);
                                        } catch (XBeeException e) {
                                            ...
                                            if (partfileLocation.toString() != null) {
                                                Process deleteparts = Runtime.getRuntime().exec(new String[]{
                                                        "bash", "-c", "rm -rf " + partfileLocation.toString().substring
                                                        (0, partfileLocation.toString().lastIndexOf("/")) +
                                                        "; rm -rf " + uploadfilesLocation + "/" + filetosend + ".aes"});
                                            ...
                                            while (i < PARTFooterbytebuffer.length) {
                                                myDevice.sendData(remote, Arrays.copyOfRange(
                                                        PARTFooterbytebuffer, i,i + chunklength));
                                                i = i + chunklength;
                                            }

                                            //apend message to log file
                                            Process appendtologfile = Runtime.getRuntime().exec(new String[]{
                                                    "bash", "-c", "echo '" + myDevice.getNodeID() + ": File sent: " +
                                                    filetosend + "' >> " + configfilesLocation +
                                                    "/" + remote.getNodeID()});
                                    ...
                                    int partstosend = Integer.parseInt(file2string(configfilesLocation
                                            + "/filecount").replace("\n", ""));
                                    ...
                                    try {
                                        ...
                                        System.err.println("CURRPART IS NOT ONE. Error transmitting message: "
                                                + e.getMessage());
                                        ...
                                        if (partfileLocation.toString() != null) {
                                            Process deleteparts = Runtime.getRuntime().exec(new String[]{
                                                    "bash", "-c", "rm -rf " + partfileLocation.toString().substring
                                                    (0, partfileLocation.toString().lastIndexOf("/")) + "; rm -rf "
                                                    + uploadfilesLocation + "/" + filetosend + ".aes"});
                            ...
                            System.out.println("Contact is unknown, please have them send you " +
                                    "their info for you to send files.");
                        }
                    }
                }
            } catch (XBeeException e) {
            ...
            try {
                ...
                if (myDevice.getNodeID().equals(REMOTE_NODE_ID)) {
                    ...
                    if (remote != null) {
                        if (getcontact(REMOTE_NODE_ID)[0].equals(REMOTE_NODE_ID)) {
                            System.out.println("Known contact, using encryption.");
                            System.out.println("Connection established. Speak to send a message," +
                                    " or type 'exit' to quit:\n");
                            while (true) {

                                if (nodeid.nextLine().substring(0, 4).contentEquals("exit")) {
                                    return;
                                }
                                ...
                                Process deepspeech = Runtime.getRuntime().exec(new String[]{
                                        "bash", "-c", "cd " + deepspeechLocation + " ; source " +
                                        "deepspeechenv/bin/activate; deepspeech --model " + deepspeechModelsLocation +
                                        "/output_graph.pb --audio RecordAudio.wav > transcription"});

                                deepspeech.waitFor();

                                System.out.println("You said: " + file2string(
                                        deepspeechLocation + "/transcription"));
                                ...
                                Process sharedsecretfile = Runtime.getRuntime().exec(new String[]{
                                        "bash", "-c", "openssl pkeyutl -derive -inkey " + privatekeysLocation.toString()
                                        + "/" + REMOTE_NODE_ID + "myprivatekey.pem -peerkey " +
                                        publickeysLocation.toString() + "/" + REMOTE_NODE_ID + "publickey.pem | openssl" +
                                        " sha3-256 | sed -e 's|(stdin)= ||' > " + userDirectory + "/sharedsecret"});
                                ...
                                while (i < CHATHeaderbytebuffer.length) {
                                    myDevice.sendData(remote, Arrays.copyOfRange(CHATHeaderbytebuffer,
                                            i, i + chunklength));
                                    //System.out.println("Sending file info...");
                                    i = i + chunklength;
                                }
                                ...
                                while (i < CHATFooterbytebuffer.length) {
                                    myDevice.sendData(remote, Arrays.copyOfRange(CHATFooterbytebuffer,
                                            i, i + chunklength));
                                    i = i + chunklength;
                                }
            ...
            System.out.println("Commands available:\n\ngenkeys: Used to generate your keypair.\nsendmyinfo: " +
                    "Sends your keypair to another device, so they can contact you in an encrypted manner.\nsendfile:" +
                    " Allows you to send an encrypted file if they have used 'sendmyinfo' to your device previously." +
                    "\nchat: Allows you to chat back and forth with encryption (if it is with a known contact)" +
                    " with another device.\nchangeid: Change your contact ID. You can type in \"random\" to generate" +
                    " a random ID.\nlistnodes: List the nodes within your PAN ID.\ndeepspeech: utilize Mozilla" +
                    " Deepspeech to transcribe audio and send as encrypted text to desired device.\nhelp: Display " +
                    "this help text.\nexit: Quit program.");
        } else if (command.equals("exit")) {
            System.exit(0);
        } //end commands
    ...
    public static void changeid(XBeeDevice myDevice) throws InterruptedException {
            ...
            if (NEW_NODE_ID.equals("random")) {
                ...
                try {
                    Process randomnodeid = Runtime.getRuntime().exec(new String[]{
                            "bash", "-c", "head /dev/urandom | tr -dc A-Za-z0-9 | head -c 13 > " +
                            configfilesLocation.toString() + "/mynodeid"});
    ...
    public static void sendparts(XBeeDevice myDevice, RemoteXBeeDevice remote, int currpart, int partstosend,
                                 String filetosend, Path partfileLocation) throws IOException, ParseException,
            GeneralSecurityException, InterruptedException, XBeeException {
        ...
        try {
            ...
            while (currpart <= partstosend) {
                ...
                Process currpartwrite = Runtime.getRuntime().exec(new String[]{
                        "bash", "-c", "echo '" + currpart + "' > " + uploadfilesLocation + "/dir-" +
                        filetosend + "/lastpartsent"});
                ...
                if (currpart < 10) {
                    partfileLocation = Paths.get(uploadfilesLocation + "/dir-" + filetosend +
                            "/" + filetosend + ".aes.7z.00" + currpart);

                } else if (!(currpart < 10) && currpart < 100) {
                    partfileLocation = Paths.get(uploadfilesLocation + "/dir-" + filetosend +
                            "/" + filetosend + ".aes.7z.0" + currpart);

                } else {
                    partfileLocation = Paths.get(uploadfilesLocation + "/dir-" + filetosend +
                            "/" + filetosend + ".aes.7z." + currpart);
                ...
                final String substring = partfileLocation.toString().substring(
                        partfileLocation.toString().lastIndexOf(filetosend));

                System.out.println(substring + " sent. \u2713");

                //SENDING PARTFOOTER
                String PARTFooter = substring + "-----ZIPPART END-----";
                byte[] PARTFooterbytebuffer = PARTFooter.getBytes();
                i = 0;
                while (i < PARTFooterbytebuffer.length) {
            ...
            while (!wasfound) {
                ...
                while (i < devices.size()) {
                    if (devices.get(i).getNodeID().equals(remote.getNodeID())) {
                        //System.out.println("Node found, restarting transfer.");
                        //Send next part here
                        //Which was the last part sent?
                        String lastpart = file2string(uploadfilesLocation + "/dir-" +
                                filetosend + "/lastpartsent");
            ...
            //Delete folder containing the parts
            Process deleteparts = Runtime.getRuntime().exec(new String[]{
                    "bash", "-c", "rm -rf " + partfileLocation.toString().substring(0, partfileLocation.toString().
                    lastIndexOf("/")) + "; rm -rf " + uploadfilesLocation + "/" + filetosend + ".aes"});
            deleteparts.waitFor();
            //close device
        }
    }

    public static List<RemoteXBeeDevice> listnodes(XBeeDevice myDevice) throws InterruptedException, IOException {
        ...
        } catch (XBeeException | JsonException e) {
            System.err.println("Error transmitting message: " + e.getMessage());
            myDevice.close();
            System.exit(1);
        } finally {
    ...
    public static void genkeys(int state, String nodeid) throws InterruptedException, JsonException {
    ...
    public static void addContact(String nodeid, String uname, String generator, String pubkey) throws IOException {
    ...
    public static Boolean fileexists(String filePath) throws IOException {
        try {
            ...
            Process checkfile = Runtime.getRuntime().exec(new String[]{
                    "bash", "-c", "FILE=" + filePath + "; if [ -f \"$FILE\" ]; then echo \"1\" ; else echo \"0\" ; " +
                    "fi >" + configfilesLocation.toString() + "/fileexists"});
            checkfile.waitFor();
    ...
    public static String[] getcontact(String nodeid) throws IOException, JsonException {
                ...
                if (messages.size() > 0) {
                    ...
                    if (messages.get(linecounter).contains("CHAT BEGIN")) {
                        ...
                        // GENERATOR KEY CATCH
                            ...
                            messages.get(linecounter).contains("-----END DH PARAMETERS-----") && (!AESSignal)) {
                        tempgenerator = messages.get(linecounter);
                        ...
                        temppubkey = (
                                messages.get(linecounter - 1) +
                                        messages.get(linecounter)
                        );
                    ...
                    if (AESSignal) {
                        if (temppubkey != null) {
                            ...
                            try {
                                sharedsecretfile = Runtime.getRuntime().exec(new String[]{
                                        "bash", "-c", "openssl pkeyutl -derive -inkey " + configfilesLocation.toString()
                                        + "/" + "myprivatekey.pem -peerkey " + configfilesLocation.toString() +
                                        "/contactpublickey.pem | openssl sha3-256 | sed -e 's|(stdin)= ||' > " +
                                        userDirectory + "/sharedsecret"});

                                sharedsecretfile.waitFor();

                            } catch (IOException | InterruptedException e) {
                                System.err.println("Something went wrong while executing openssl.");
                                e.printStackTrace();
                            }

                            sharedsecret = file2string(userDirectory + "/sharedsecret");
                        ...
                        if (messages.get(linecounter).contains("CHAT END")) {
                            ...
                            if (fileexists(downloadfilesLocation + "/msg")) {
                                try {
                            ...
                            if (fileexists(downloadfilesLocation + "/msg.aes")) {
                                try {
                                    Process remold = Runtime.getRuntime().exec(new String[]{
                            ...
                            System.out.println(xbeeMessage.getDevice().getNodeID() + ": " +
                                    file2string(downloadfilesLocation + "/msg"));
                    ...
                    if (ZIPFILESignal) {
                        ...
                        if (temppubkey != null) {
                            try {
                                ...
                                cleanpubkey = cleanpubkey.replace("?", "\\n");
                                Process pubkeymake = Runtime.getRuntime().exec(new String[]{
                                        "bash", "-c", "echo '" + cleanpubkey + "' > " + configfilesLocation.toString()
                                        + "/contactpublickey.pem; cat " + configfilesLocation.toString() +
                                        "/contactpublickey.pem  | sed -i 's/\\\\n/\\'$'\\n''/g' " +
                                        configfilesLocation.toString() + "/contactpublickey.pem"});
                            ...
                            try {
                                ...
                                System.err.println("Something went wrong while trying to use openssl.");
                                e.printStackTrace();
                            }
                            ...
                            try {
                                ...
                                System.err.println("Something went wrong while removing /"
                                        + userDirectory + "/sharedsecret.");
                                e.printStackTrace();
                            }
                        ...
                        if (ZIPPARTSignal) {
                            ...
                            if (messages.get(linecounter).contains("ZIPPART END")) {
                                ...
                                if ((messages.get(linecounter).lastIndexOf("-----ZIPPART END-----")) >= 0) {
                                    ZIPFilename = messages.get(linecounter).substring
                                            (0, messages.get(linecounter).lastIndexOf("-----ZIPPART END-----"));
                                ...
                                if (fileexists(downloadfilesLocation + "/" + ZIPFilename.substring
                                        (0, ZIPFilename.lastIndexOf(".aes.7z.") + 8) + "/" + ZIPFilename)) {
                                    try {
                                        Process remold = Runtime.getRuntime().exec(new String[]{
                                                "bash", "-c", "rm -rf " + downloadfilesLocation + "/" +
                                                ZIPFilename.substring(0, ZIPFilename.lastIndexOf(".aes.7z.") + 8)
                                                + "/" + ZIPFilename});
                                        remold.waitFor();
                                    } catch (IOException | InterruptedException e1) {
                                        System.out.println("Error removing old file.");
                                ...
                                try {
                                    Process mkfile = Runtime.getRuntime().exec(new String[]{
                                            "bash", "-c", "mkdir " + downloadfilesLocation + "/" +
                                            ZIPFilename.substring(0, ZIPFilename.lastIndexOf(".aes.7z.") + 8) + "/"
                                            + ";touch " + downloadfilesLocation + "/" + ZIPFilename.substring
                                            (0, ZIPFilename.lastIndexOf(".aes.7z.") + 8) + "/" + ZIPFilename});
                                    mkfile.waitFor();

                                    //write byte array to file
                                    try (FileOutputStream stream = new FileOutputStream(downloadfilesLocation
                                            + "/" + ZIPFilename.substring(0, ZIPFilename.lastIndexOf(".aes.7z.")
                                            + 8) + "/" + ZIPFilename)) {
                                        for (i = 0; i < tempfile.length; i++) {
                                            for (int j = 0; j < tempfile[i].length; j++) {
                                                stream.write(tempfile[i][j]);
                                    ...
                                    System.err.println("Something went wrong while removing file. " + e1.getMessage());
                                }
                                ...
                                try {
                                    Process countfiles = Runtime.getRuntime().exec(new String[]{
                                            ...
                                            "bash", "-c", "ls -1a " + downloadfilesLocation + "/" + ZIPFilename
                                            .substring(0, ZIPFilename.lastIndexOf(".aes.7z.") + 8) + "/ | wc -l > "
                                            + configfilesLocation + "/filecount"});
                                    countfiles.waitFor();
                                ...
                                // Remove trailing zero bytes from the last ZIPFile received,
                                // remove end zero bytes and then add 1 zero byte.


                                //System.out.println("*******************REMOVING ZEROS*******************.");
                                try {
                                    Process remzeros = Runtime.getRuntime().exec(new String[]{
                                            "bash", "-c", "sed '$ s/\\x00*$//' " + downloadfilesLocation + "/" +
                                            ZIPFilename.substring(0, ZIPFilename.lastIndexOf(".aes.7z.") + 8)
                                            + "/" + ZIPFilename + " > "
                                            + downloadfilesLocation + "/" + ZIPFilename.substring(0, ZIPFilename.
                                            lastIndexOf(".aes.7z.") + 8) + "/" + ZIPFilename + ".stripped ; rm -rf "
                                            + downloadfilesLocation + "/" + ZIPFilename.substring(0, ZIPFilename.
                                            lastIndexOf(".aes.7z.") + 8) + "/" + ZIPFilename + "; mv "
                                            + downloadfilesLocation + "/" + ZIPFilename.substring(0, ZIPFilename.
                                            lastIndexOf(".aes.7z.") + 8) + "/" + ZIPFilename + ".stripped "
                                            + downloadfilesLocation + "/" + ZIPFilename.substring(0, ZIPFilename.
                                            lastIndexOf(".aes.7z.") + 8) + "/" + ZIPFilename});
                                ...
                                System.out.println("Part file received: " + numfiles + "/" + fileparts);

                                if ((numfiles == fileparts)) {
                                    //System.out.println("*******************START ADDING ZEROS*******************.");
                                    //	IF FILE IS THE LAST FILE (NEEDS 0 BYTE AT THE END)
                                    try {
                                        Process addzeros = Runtime.getRuntime().exec(new String[]{
                                                "bash", "-c", "truncate -s +2 " + downloadfilesLocation + "/" +
                                                ZIPFilename.substring(0, ZIPFilename.lastIndexOf(".aes.7z.") + 8)
                                                + "/" + ZIPFilename});
                                        addzeros.waitFor();
                                    ...
                                    try {
                                        Process unzip = Runtime.getRuntime().exec(new String[]{
                                                "bash", "-c", "cd " + downloadfilesLocation + "/" +
                                                ZIPFilename.substring(0, ZIPFilename.lastIndexOf(".aes.7z.") + 8)
                                                + "/; 7z x " + ZIPFilename});
                                        unzip.waitFor();
...
//											DECRYPT AESCrypt file
                                    ...
                                    //Converting aes file to original with shared sha3-256 secret
                                    System.out.println("File received: " + AESFilename.substring
                                            (0, AESFilename.length() - 4));
                                    try {
                                        decrypter.decrypt(
                                                downloadfilesLocation + "/" + ZIPFilename.substring(0, ZIPFilename.
                                                        lastIndexOf(".aes.7z.") + 8) + "/" + AESFilename,
                                                downloadfilesLocation + "/" + AESFilename.substring(0, AESFilename.
                                                        length() - 4));
                                    } catch (IOException | GeneralSecurityException e) {
                                        // TODO Auto-generated catch block
                                        e.printStackTrace();
                                    ...
                                    Process appendtologfile = Runtime.getRuntime().exec(new String[]{
                                            "bash", "-c", "echo '" + xbeeMessage.getDevice().getNodeID() + ": " +
                                            ("File sent: " + AESFilename.substring(0, AESFilename.length() - 4)) +
                                            "' >> " + configfilesLocation + "/" + xbeeMessage.getDevice().getNodeID()});
                                    appendtologfile.waitFor();
                    ...
                    if (messages.get(linecounter).contains("-----ZIPFILE END-----")) {
                        ...
                        Process deleteparts = Runtime.getRuntime().exec(new String[]{
                                "bash", "-c", "rm -rf " + downloadfilesLocation + "/" + ZIPFilename.substring
                                (0, ZIPFilename.lastIndexOf(".aes.7z.") + 8)});
                        deleteparts.waitFor();
                        connected = false;
                ...
                System.err.println("While zipping or unzipping a file, an exception occurred.");
                e.printStackTrace();
            } finally {
                linecounter += 1;
                prevmsgtimestamp = secondspassed;
                //if (AESSignal == false) {
                //if ( !(xbeeMessage.getDataString().contains("PUBLIC KEY-----")) &&
                //     !(xbeeMessage.getDataString().contains("-----AES END-----")) ) {
                ...
                if (!(tempcontact[0][0].isBlank() && (tempcontact[1][0].isBlank()) &&
                     (tempcontact[2][0].isBlank()) && (tempcontact[3][0].isBlank()))) {
                    try {
